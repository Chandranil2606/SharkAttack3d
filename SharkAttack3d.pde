...
void setup()
{
  ...
  //Code for using in Kinect here!
  size(800, 800, P3D);         // Android
  frameRate(30);
  //cam = new PeasyCam(this, 1000);
  tree = new Tree(new PVector(0,0), new PVector(0, 1));
  tree2 = new Tree(new PVector(10,height/6), new PVector(0, -1));
...
void draw() 
{
  ...
  // Add corals and plants here.
  
  scale(0.5);
  // Plant
   tree.show();
   tree2.show();
  if(tree.current_number_of_leaves< 25){
    System.out.println(tree.current_number_of_leaves);
  tree.grow();
  }
  if(tree2.current_number_of_leaves< 25){
    System.out.println(tree2.current_number_of_leaves);
  tree2.grow();
  }
  scale(0.3);
